...
import { Component404, getCookies, PrismicLink } from './index';
...
async function makeIntrospectionQuery(
  uri: string,
  repositoryName: string,
  accessToken: string
): Promise<any> {
...
export function withPreview<P extends object>(ComposedComponent?: any) {
  return class extends React.Component<IPreviewProps, IPreviewState> {
    constructor(props: IPreviewProps) {
      super(props);
      const prismicInfos =
        typeof window !== 'undefined' ? (window as any).___sourcePrismicGraphql : {};
      this.state = {
        // proxy data to state
    ...
    buildPreviewQuery() {
      const {
        _PRISMIC_PREVIEW_QUERY_FN_,
        _PRISMIC_PREVIEW_QUERY_,
        _PRISMIC_URL_PATTERN_,
        _PRISMIC_CUSTOM_TYPE_,
    ...
    async queryDoc(
    ...
    ): Promise<[ReadonlyArray<GraphQLError> | null, any | null]> {
      ...
      else if (res.data && prefix)
        return [null, res.data[prefix].edges.length > 0 ? res.data[prefix].edges[0].node : null];
      else return [null, null];
    }

    load = async (variables?: { [key: string]: string }) => {
      const client = await getClient(
        this.state.prismicInfos.repositoryName,
        this.state.prismicInfos.accessToken
      );
      try {
    ...
    render() {
      ...
      if (!ComposedComponent || !this.state.data) {
        ...
        if (Component404 && (Component404 as any).default) {
          ...
          if (typeof window !== 'undefined') {
            (window as any).location = '/404.html';
          }
        }
      }
      return (
